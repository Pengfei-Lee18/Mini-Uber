    ...
    if request.method == 'POST':
        ...
        if cur_user.driver:
            ...
            if isdriver == None and (ridelist):
                ...
                return render(request, 'login/editprofile.html', locals())
            print(isdriver)
            print(cartype == cur_car.cartype, plateNumber == cur_car.plateNumber \
            , passengersNumber==str(cur_car.passengersNumber), freeText == cur_car.freeText)
            if (not (cartype == cur_car.cartype and plateNumber == cur_car.plateNumber \
            and passengersNumber == str(cur_car.passengersNumber) and freeText == cur_car.freeText)) and (ridelist):
    ...
    if request.method == 'POST':
        ...
        if sharer_form.is_valid():
            ...
            if(latetime<earlytime):
                message = "time must be a valid range"
...
def shareredit(request, ride_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return render(request, 'login/shareredit.html', locals())
    return render(request, 'login/shareredit.html', locals())
